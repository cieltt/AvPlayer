...
JNIEXPORT void JNICALL
Java_com_example_ciel_AvPlayer_player_DNPlayer_native_1set_1surface(JNIEnv *env, jobject instance,
                                                               jobject surface) {
    pthread_mutex_lock(&mutex);
    //先释放之前的显示窗口
    if (window) {
...
Java_com_example_ciel_AvPlayer_player_DNPlayer_native_1getDuration(JNIEnv *env, jobject instance) {
    if (ffmpeg) {
